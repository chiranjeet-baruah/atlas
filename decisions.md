...
2. **Per-attempt LLM timeout.** `modelclient.Client.Extract`'s 3 retries used to share one deadline, so a slow first attempt could eat the whole budget and leave nothing for attempts 2 and 3. Now each attempt gets its own `constants.LLMAttemptTimeout`. That constant was measured, not guessed: 3 real calls to qwen3 with a real (short) resume took 36.9s/42.4s/45.7s, generating 888-1147 completion tokens — it's a reasoning model, most of that time is chain-of-thought before the JSON comes out. The instinctive "30s should be plenty" guess would have failed almost every attempt; landed on 60s instead, ~15s of margin over the worst observed call on a *short* resume. Longer real resumes may need this re-measured.
