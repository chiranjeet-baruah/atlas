...
func (r *Repository) SaveExtraction(ctx context.Context, id string, rawText string, fields domain.ExtractedFields) error {
	...
	_, err = r.pool.Exec(ctx, `
		UPDATE resumes
		SET raw_text = $1, skills = $2, years_experience = $3, location = $4, extracted_json = $5, updated_at = now()
		WHERE id = $6
	`, rawText, nonNilSkills(fields.Skills), nullFloat(fields.YearsExperience), fields.Location, extractedJSON, id)
	if err != nil {
		return fmt.Errorf("save extraction for resume %s: %w", id, err)
	}
	return nil
}
...
func (r *Repository) GetByID(ctx context.Context, id string) (domain.Resume, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT id, batch_id, filename, file_path, status, COALESCE(error_message,''),
...
func (r *Repository) Search(ctx context.Context, queryVec []float32, filters domain.SearchFilters, limit int) ([]domain.SearchResult, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT r.id, r.batch_id, r.filename, r.file_path, r.status, COALESCE(r.error_message,''),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&res.ID, &res.BatchID, &res.Filename, &res.FilePath, &res.Status, &res.ErrorMessage,
...
func scanResume(row rowScanner) (domain.Resume, error) {
	var res domain.Resume
	err := row.Scan(&res.ID, &res.BatchID, &res.Filename, &res.FilePath, &res.Status, &res.ErrorMessage,
