...
- `Makefile`'s `test-unit` target hand-enumerates packages; `./internal/adapter/driver/web/...` must be added to it (Task 6) or its tests silently never run in CI.
...
### Task 6: Wire into serve.go, update Makefile, integration smoke test
